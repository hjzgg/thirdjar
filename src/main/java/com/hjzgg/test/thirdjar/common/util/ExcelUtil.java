...
import com.hjzgg.test.thirdjar.common.util.annotation.Excel;
import com.hjzgg.test.thirdjar.common.util.annotation.ExcelExtend;
...
//    }
    ...
    private static void gatherColumnWidths(Map<Integer, Integer> columnWidths, String headerName, String content, int j) {
        int length = content.getBytes(Charset.defaultCharset()).length;
        if( headerName.equalsIgnoreCase(content) ){
            if( headerName.indexOf("（")>0 ){
                ...
                length = lengthStr.getBytes(Charset.defaultCharset()).length;
            }
